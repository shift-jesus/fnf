...
typedef TitleData =
{
	...
	// ============================================================
	//  CAMPOS EXTRA PARA FONDO DE TITULO ANIMADO
	// ------------------------------------------------------------
	//  backgroundAnim : prefijo que comparten los frames del atlas.
	//                   Ej: si el .xml tiene "Fondo 10000" y "Fondo 10001",
	//                   el prefijo es "Fondo ". Se usa con addByPrefix().
	//  backgroundFps  : velocidad de la animacion en fps (opcional).
	// ============================================================
	@:optional var backgroundAnim:String;
	@:optional var backgroundFps:Int;
}
		...
		if(Paths.fileExists('images/gfDanceTitle.json', TEXT))
		{
			...
			if(titleRaw != null && titleRaw.length > 0)
			{
				try
				{
					...
					// ------------------------------------------------------------
					//  Como cambiarlo (configurado en images/gfDanceTitle.json):
					//
					//  FONDO ANIMADO  -> "backgroundSprite": "fondo"
					//       Necesita fondo.png + fondo.xml juntos en la misma
					//       carpeta. El .xml define los frames del atlas.
					//       - "backgroundAnim" = prefijo comun de los frames
					//         (ej: "Símbolo 1 instancia ").
					//       - "backgroundFps"  = velocidad de la animacion.
					//       Si el .xml existe, se carga como atlas y se anima.
					//
					//  FONDO ESTATICO -> "backgroundSprite": "sonic"
					//       Solo un .png normal (sin .xml al lado).
					//       Se muestra como imagen quieta.
					// ============================================================
					if (titleJSON.backgroundSprite != null && titleJSON.backgroundSprite.trim().length > 0)
					{
						...
						// Si hay un .xml hermano, el fondo es un atlas animado
						if (Paths.fileExists('images/' + titleJSON.backgroundSprite + '.xml', TEXT))
						{
							bg.frames = Paths.getSparrowAtlas(titleJSON.backgroundSprite);

							var bgAnimPrefix:String = (titleJSON.backgroundAnim != null && titleJSON.backgroundAnim.length > 0) ? titleJSON.backgroundAnim : '';
							var bgFps:Int = (titleJSON.backgroundFps != null) ? titleJSON.backgroundFps : 24;

							// Prefijo vacio = usar todos los frames del atlas en orden
							if (bgAnimPrefix.length > 0)
								bg.animation.addByPrefix('bganim', bgAnimPrefix, bgFps, true);
							else
								bg.animation.addByIndices('bganim', [for (i in 0...bg.frames.frames.length) i], '', bgFps, true);

							bg.animation.play('bganim');
						}
						else // Sin .xml = imagen estatica
						{
							bg.loadGraphic(Paths.image(titleJSON.backgroundSprite));
						}
						add(bg); // Se agrega primero = queda detras de todo
					}
				}
